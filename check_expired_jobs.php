...
require_once __DIR__ . '/Model/Job.php'; // Assuming Job model exists
...
echo "Starting check_expired_jobs.php at " . date('Y-m-d H:i:s') . "\n";
error_log("Starting check_expired_jobs.php at " . date('Y-m-d H:i:s'));
...
try {
    ...
    $errorMsg = "Failed to initialize database or Job model: " . $e->getMessage();
    echo $errorMsg . "\n";
    error_log($errorMsg);
    exit(1);
}
...
while (true) {
    try {
        ...
        if (isEndOfDay()) {
            echo "Running checkAllExpiredJobs at " . date('Y-m-d H:i:s') . "\n";
            error_log("Running checkAllExpiredJobs at " . date('Y-m-d H:i:s'));

            // Call the method to check and update expired jobs
            $job->checkAllExpiredJobs();

            echo "Finished checking expired jobs at " . date('Y-m-d H:i:s') . "\n";
            error_log("Finished checking expired jobs at " . date('Y-m-d H:i:s'));
        ...
        $errorMsg = "Error in check_expired_jobs script: " . $e->getMessage();
        echo $errorMsg . "\n";
        error_log($errorMsg);
