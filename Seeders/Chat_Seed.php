...
trait Chat_Seed {
    ...
    private function seedChat1(): void {
        ...
        $stmt = $this->db->prepare("INSERT INTO chats (user_id, receiver_id, latest_message) VALUES (?, ?, ?)");
        $stmt->execute([1, 2, 'Good, lets meet up']);
    ...
    private function seedChat2(): void {
        ...
        $stmt = $this->db->prepare("INSERT INTO chats (user_id, receiver_id, latest_message) VALUES (?, ?, ?)");
        $stmt->execute([2, 3, 'Wait me up, I  am on the way.']);
